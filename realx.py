...
from sample import Random_Bernoulli_Sampler, REBAR_Bernoulli_Sampler
...
class REALX:
    def __init__(self, selector, predictor, lamda, output_channels=1):
        ...
        self.resize_layer = ResizeMask(
            self.input_shape, self.mask_size, self.output_channels
        )
        ...
        self.evalx = keras.models.clone_model(predictor)
        self.evalx._name = "evalx"
        self.build_evalx()
        self.has_evalx = False

    def build_predictor(self):
        model_input = keras.layers.Input(shape=(*self.input_shape,), dtype="float32")

        sample_input = keras.layers.Flatten()(model_input)
        sample_input = keras.layers.Lambda(lambda x: x[:, : self.mask_size])(
            sample_input
        )
        r = Random_Bernoulli_Sampler()(sample_input)
        r = self.resize_layer(r)
        masked_input = keras.layers.Multiply()([model_input, r])

        preds = self.predictor(masked_input)

        model = keras.models.Model(model_input, preds)
        model.summary()
    ...
    def build_selector(self):
        self.selector = SELECTOR(
            self.selector,
            self.predictor.get_layer("predictor"),
            self.lamda,
    ...
    def evaluate(self, x, batch_size):
        if not self.has_evalx:
            ...
            _, s, _ = REBAR_Bernoulli_Sampler(tau0=0.1)(logits)
            s = self.resize_layer(s)
            masked_input = keras.layers.Multiply()([model_input, s])
            preds = self.evalx.get_layer("evalx")(masked_input)

            self.evalx_model = keras.models.Model(model_input, preds)
    ...
    def select(self, x, batch_size, discrete=False):
        ...
        if not self.has_selector:
            model_input = keras.layers.Input(
                shape=(*self.input_shape,), dtype="float32", name="ecg"
            )
            logits = self.selector.selector(model_input)
            s = keras.layers.Activation(keras.activations.sigmoid)(logits)
            s = self.resize_layer(s)

            self.selector_model = keras.models.Model(model_input, s)
        ...
        if discrete:
            explainations = (explainations > 0.5).astype(int)
        else:
...
class SELECTOR(keras.Model):
    ...
    def build_predictor(self, predictor):
        ...
        s_in = keras.layers.Input(shape=(self.mask_size,), dtype="float32")

        s = self.resize_layer(s_in)
        masked_input = keras.layers.Multiply()([model_input, s])
        y_pred = predictor(masked_input)

        model = keras.models.Model([model_input, s_in], y_pred)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            z, s, z_tilde = REBAR_Bernoulli_Sampler(tau0=0.1)(sel_prob)
            ...
            # 1. f(s)
            p_f_s = keras.ops.sum(y_batch * K.log(f_s + 1e-5), axis=1)
            # 2. c(z)
            p_c_z = keras.ops.sum(y_batch * K.log(c_z + 1e-5), axis=1)
            # 3. c(z~)
            p_c_z_tilde = keras.ops.sum(y_batch * K.log(c_z_tilde + 1e-5), axis=1)
            # 4. q(s)
            sel_prob = K.sigmoid(sel_prob)
            q_s = keras.ops.sum(
                s * K.log(sel_prob + 1e-5) + (1 - s) * K.log(1 - sel_prob + 1e-5),
                axis=1,
            )

            # Compute the Sparisity Regularization
            # 1. R(s)
            R_s = keras.ops.mean(s, axis=1)
            R_s_approx = keras.ops.mean(sel_prob, axis=1)
            ...
            self.optimizer.apply_gradients(
                zip(sf_grads, self.selector.trainable_variables),
                experimental_aggregate_gradients=False,
            )

        # Calculate Objective Loss
        loss = keras.ops.mean(p_f_s - self.lamda * (R_s))
        obj_dict = {"loss": loss, "sel%": keras.ops.mean(R_s)}
...
class ResizeMask(layers.Layer):
    def __init__(self, in_shape, mask_size, output_channels=1, **kwargs):
        ...
        self.output_channels = output_channels
        self.reshape_shape, self.resize_aspect, self.pad_shape = (
            self.get_reshape_shape()
        )
    ...
    def call(self, inputs):
        ...
        if len(self.in_shape) == 1:
            ...
            if self.pad_shape > 0:
                out = keras.layers.Lambda(
                    lambda x: x[:, self.pad_shape : -self.pad_shape]
                )(out)

        else:
            # Reshape
            out = keras.layers.Reshape(tuple(self.reshape_shape))(inputs)
            ...
            if len(self.pad_shape) == 1:
                out = keras.layers.Lambda(
                    lambda x: x[:, self.pad_shape[0] : -self.pad_shape[0], :]
                )(out)
